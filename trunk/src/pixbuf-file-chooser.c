...
static GSList* 			pixbuf_get_savable_formats		(void);
...
save_response_cb (GtkDialog *dlg, gint id, gpointer data)
{
	...
	my_format 	= pixbuf_file_chooser_get_format (PIXBUF_FILE_CHOOSER (dlg));
	...
	if (format != my_format)
	{
		...
		gtk_file_chooser_set_current_name ( GTK_FILE_CHOOSER (dlg), new_file_name);
...
pixbuf_file_chooser_add_filter (PixbufFileChooser *chooser)
{
	...
	{
		formats = pixbuf_get_savable_formats ();
	}
	else
	{
		/* All Files Filter */
...
pixbuf_file_chooser_set_current_filter (PixbufFileChooser *chooser, const gchar *extension)
{
	...
	if (format != NULL)
	{
		GSList *it;
		GSList *list = NULL;
		list =  gtk_file_chooser_list_filters ( GTK_FILE_CHOOSER (chooser));
		for (it = list; it != NULL; it = it->next)
		{
			GtkFileFilter *filter;
			GdkPixbufFormat* it_format;
			filter = (GtkFileFilter*)it->data;
...
static GSList*
pixbuf_get_savable_formats (void)
{
	GSList *list;
