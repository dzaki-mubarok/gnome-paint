...
draw_in_pixmap ( GdkDrawable *drawable )
{
	...
	{
		gdk_draw_rectangle (drawable, gcb, TRUE, MIN(x0,x1), MIN(y0,y1), ABS(x1-x0), ABS(y1-y0));
	}
	else
	if ( cv->filled == FILLED_FORE )
	{
		gdk_draw_rectangle (drawable, gcf, TRUE, MIN(x0,x1), MIN(y0,y1), ABS(x1-x0), ABS(y1-y0));
	}

	if ( cv->filled != FILLED_FORE )
	{
		gdk_draw_rectangle (drawable, gcf, FALSE, MIN(x0,x1), MIN(y0,y1), ABS(x1-x0), ABS(y1-y0));
	}
}
